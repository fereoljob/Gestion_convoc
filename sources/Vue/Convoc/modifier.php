...
<div class ="liste">
    <?php
        echo "<br/>";
        $convok = $donnees["convo"];
        $nb = 14-$convok["nbre_convok"];
        echo "<h1>Encore $nb joueurs maximum a ajouter a cette convocation</h1>";
        echo '<form id=myform method=post action=Convoc/modif >';
        echo "<table>";
        ...
        echo " />";
        echo '<input type=hidden name=convol value=';
        echo $donnees["convo"]["id_convocation"];
        echo " />";
        echo '</form>';
    ?>
    <script>
            ...
            monform.addEventListener('submit',function(e)
            {
                ...
                if(cs.length><?php echo $nb;?>)
                {
                    e.preventDefault();
                    alert("Vous devez selectionner rigoureusement un nombre de joueurs inferieur ou égale à  <?php echo $nb;?>!");
